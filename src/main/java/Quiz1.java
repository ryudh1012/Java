...
public class Quiz1 {
    public static void main(String[] args){
        ...
        if (num % 21 == 0) { // 입력한 값을 3과 7의 최소공배수인 21로 나누어 나머지가 0이면 3과 7의 배수 모두 만족
            System.out.print("3과 7의 배수 모두 만족합니다."); //11행이 참이면 둘 다 만족함을 출력
        }
        else if (num % 3 == 0) { //입력한 값을 3으로 나누어 나머지가 0이면 3의 배수
            System.out.print("3의 배수입니다."); //14행이 참이면 3의 배수임을 출력
        }
        else if (num % 7 == 0) { //입력한 값을 7로 나누어 나머지가 0이면 7의 배수
            System.out.print("7의 배수입니다."); //17행이 참이면 7의 배수임을 출력
        }
        else { //입력한 값이 3과 7의 배수 모두 만족하지 않을 경우
